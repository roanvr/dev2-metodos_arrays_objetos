...
dental.forEach((x)=>{
    document.write(`<p>${x.hora} - ${x.especialista} - ${x.paciente} - ${x.rut} - ${x.prevision}</p>`)
})

let allPatients = radiologia.concat(traumatologia, dental)
console.log(allPatients);

allPatients.forEach(function(x){
    document.write(`<p>${x.paciente}</p>`)
});

let totalIsapre = dental.filter(function(x){
    if(x.prevision == 'ISAPRE') {
        document.write(`<p>${x.paciente} - ${x.prevision}</p>`)
    }
});

let totalFonasa = dental.filter(function(x){
